...
- classifier v2 先在本地检查“额度主题 + 重置动作”，不满足时不会调用 DeepSeek；每帖最多生成一个主事件。
- AI 证据必须逐字来自当前帖子，历史上下文不能证明当前事件。分类以整帖语义为主：Tibo 的试探式问句或社交媒体预热可以构成“已预告”信号；已经执行但仍需几分钟生效的重置归为“已完成”。代码只保留主题候选门和原文证据校验，不再用固定时态正则覆盖模型结论。
...
- 收件邮箱使用可添加、逐个删除的列表；测试邮件和正式提醒会发送给列表中的全部地址。
